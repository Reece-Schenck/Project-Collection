...
void SoundManager::addObserver(SoundObserver* observer) {
	observers.push_back(observer);
}

void SoundManager::removeObserver(SoundObserver* observer) {
	// Implement removal logic later if needed
}

void SoundManager::notifyFire(Vector2f emitterLocation, Vector2f listenerLocation) {
	for (auto observer : observers) {
		observer->notifyFire(emitterLocation, listenerLocation);
	}
}

void SoundManager::notifyFallInFire() {
	for (auto observer : observers) {
		observer->notifyFallInFire();
	}
}

void SoundManager::notifyFallInWater() {
	for (auto observer : observers) {
		observer->notifyFallInWater();
	}
}

void SoundManager::notifyJump() {
	for (auto observer : observers) {
		observer->notifyJump();
	}
}

void SoundManager::notifyReachGoal() {
	for (auto observer : observers) {
		observer->notifyReachGoal();
	}
}
