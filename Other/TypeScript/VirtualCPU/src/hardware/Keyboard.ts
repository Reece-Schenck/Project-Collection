...
export class Keyboard extends Hardware implements interrupt{
    ...
    // not needed as keyboard only provides output
    inputBuffer: string[];

    constructor(irqNumber: number, priority: number, deviceName: string, interruptController: interruptController){
        super('Keyboard');
        this.irqNumber = irqNumber;
        this.priority = priority;
        this.deviceName = deviceName;
        this.interruptController = interruptController;
        // this.interruptController.registerInterruptSource(deviceName);

        this.monitorKeys();
    }

    provideInterruptToCPU(): interrupt {
        return this.interruptController.provideInterruptToCPU();
    }
