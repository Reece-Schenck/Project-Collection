...
export class Cpu extends Hardware implements ClockListener{
    ...
    private instructionRegister: string;
    private zFlag: number; // 0=True(the two values are equal), 1=False(the two values aren't equal)
    private lowOrder: number; // first byte -> 0xAD 10 00 --> 0x0010
    private highOrder: number; // second byte
    private temp: number; // helper for high and low order setting
    private address: number; // helper for combining high and low order
    private char: string;

    // Pipeline step
    private pipelineStep: string;
    Keyboard: Keyboard;

    constructor(name: string, mmu: Mmu, system: System) {
        super(name);
        this.log("created");
        this.mmu = mmu;
        ...
        this.char = '';
        this.ascii = new Ascii(this.char);
    }
    ...
    setAccumulator(a: number): void {
        this.accumulator = a;
    }

    setXRegister(x: number): void {
        this.xRegister = x;
    }

    setYRegister(y: number): void {
        this.yRegister = y;
    }
    ...
    setInstructionRegister(iR: string): void {
        this.instructionRegister = iR;
    }

    setZFlag(z: number): void {
        this.zFlag = z;
    }
